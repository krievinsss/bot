    ...
    <div className="card overflow-x-auto">
      <table className="table">
        ...
        <tbody>{rows.map(x=><tr key={x.id}>
          <td>{x.imageUrl?<img src={x.imageUrl} alt={x.name} className="h-14 w-14 rounded-lg object-cover border border-slate-700"/>:<span className="muted text-xs">No image</span>}</td>
